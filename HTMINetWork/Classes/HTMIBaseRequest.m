...
- (instancetype)init
{
    ...
    if (self) {
        ...
        NSString *accessToken = [defaults objectForKey:@"htmiAccessToken"] ==  nil ? @"": [defaults objectForKey:@"htmiAccessToken"];
        NSString *refreshToken = [defaults objectForKey:@"htmiRefreshToken"] ==  nil ? @"": [defaults objectForKey:@"htmiRefreshToken"];
        
        _httpHeaderDic = [NSMutableDictionary dictionaryWithDictionary:@{@"device":@"iphone",@"accessToken":accessToken,@"refreshToken":refreshToken}];
    ...
    if ([self shouldCallAPIWithParams:apiParams]) {       //通过参数决定是否发送请求
        if ([self isCorrectWithParamsData:apiParams]) {    //检查参数正确性
            ...
            if ([self isReachable]) {              // 有网络
                ...
                NSDictionary *dic  = [NSMutableDictionary dictionaryWithDictionary:@{@"device":@"iphone",@"accessToken":accessToken,@"refreshToken":refreshToken}];
                NSMutableDictionary * mDic = [NSMutableDictionary dictionaryWithDictionary:headers];
